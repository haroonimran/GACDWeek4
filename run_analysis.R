...
#_________________________________________________________________________
#STEP1: Set the workiing directory, and read train and test data sets into 'R' objects.
# STEP 1a. Set the working diirectory to the location where the train and test data set files are available.

train_data<- as.data.frame(read.delim(file = paste0(getwd(),"/UCI HAR Dataset/train/X_train.txt"),sep = "",header = FALSE)) #converting the "training" dataset into a data frame and saving into the object "train_data".
test_data <- as.data.frame(read.delim(file = paste0(getwd(),"/UCI HAR Dataset/test/X_test.txt"),sep = "",header = FALSE)) #converting the "test" dataset into a data frame and saving into the object "test_data".
...
train_labels <- as.data.frame(read.delim(file = paste0(getwd(),"/UCI HAR Dataset/train/y_train.txt"),sep = "",header = FALSE))
test_labels  <- as.data.frame(read.delim(file = paste0(getwd(),"/UCI HAR Dataset/test/y_test.txt"),sep = "",header = FALSE))
...
#_________________________________________________________________________________
#The descriptive_names vector was created manually by this author in order to map each non-descriptive nem with a descriptive one.
...
subject_train <- data.frame(read.csv(file = paste0(getwd(),"/UCI HAR Dataset/train/subject_train.txt"),header = FALSE))
subject_test  <- data.frame(read.csv(file = paste0(getwd(),"/UCI HAR Dataset/test/subject_test.txt"),header = FALSE))
...
colnames(final_tidy_dataset)[1] <- "activitylabel"
